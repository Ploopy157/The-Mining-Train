...
local function BuildPlayerTrain(
	...
	-- The drill is intentionally created after every ore car,
	-- so it is always the final child added to the train model.
	local DrillModel,
		DrillStatsOrError =
	...
	if DrillModel then
		...
		-- Continue from the final ore car so the consist is:
		-- locomotive -> ore cars -> drill.
		--
		-- PreviousLength is the final car's length after the car loop.
		-- TotalOffset is the final car's centre offset from the locomotive.
		local DrillOffset =
			TotalOffset
			+ PreviousLength / 2
			+ CarGap
	...
	elseif typeof(DrillStatsOrError) ~= "table"
		or DrillStatsOrError.IsUnlocked
			~= false then

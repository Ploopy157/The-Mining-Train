...
local function EnsurePlayerUpgradeData(Data)
	...
	if typeof(Data.Train.DrillId)
		~= "string" then

		Data.Train.DrillId = "NoDrill"
	end
...
local function GetDrillDefinitionFromLevel(
	Level
)
	local DrillId =
...
local function GetDrillNameFromLevel(Level)
	local DrillId, Definition =
		GetDrillDefinitionFromLevel(
			Level
		)
...
local function GetDrillDamageFromLevel(Level)
	local _, Definition =
		GetDrillDefinitionFromLevel(
			Level
		)
	...
	return Definition
		and Definition.Damage
		or 0
end

local function GetDrillSpeedFromLevel(Level)
	local _, Definition =
		GetDrillDefinitionFromLevel(
			Level
		)
		...
		if Definition then
			...
			table.insert(Upgrades, {
				...
				FormattedValue =
					...
					)					or UpgradeId == "DrillTier"
					and (
						IsMaximumLevel
						and (
							"Current: "
							.. GetDrillNameFromLevel(
								Level
							)
							.. " | Damage "
							.. tostring(
								GetDrillDamageFromLevel(
									Level
								)
							)
							.. " | Speed "
							.. tostring(
								GetDrillSpeedFromLevel(
									Level
								)
							)
						)
...
local function ApplyUpgrade(
	...
	elseif UpgradeId == "DrillTier" then
		local AppliedSuccessfully,
			Result =
			ApplyDrillTierUpgrade(
				Data
			)
